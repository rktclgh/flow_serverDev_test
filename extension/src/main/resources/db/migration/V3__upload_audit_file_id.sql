...
--      (외부 리뷰 Codex P1)
--
--    ★ 대소문자는 이 판별의 관심사가 아니다 — 그래서 ~ 가 아니라 ~* 다.
--
--      StorageKeyGenerator 가 소문자를 낸다는 것은 애플리케이션의 사실이지 DB 불변식이
--      아니다. V2 의 stored_key 는 제약 없는 TEXT 라 대문자 키를 담을 수 있고, 옛 데이터나
--      수동 삽입이 그런 값을 남겼을 수 있다.
--
--      대소문자를 구분하면 그런 키를 못 알아보고 gen_random_uuid() 로 보낸다. 그러면
--      이미 저장된 객체와 행의 식별자가 어긋나 다운로드가 조용히 404 가 된다 —
--      마이그레이션이 멈추지도 않아서 아무도 눈치채지 못한다. 위의 두 실패(하이픈 36개 등)
--      가 시끄럽게 터지는 것과 달리 이쪽은 조용해서 더 위험하다.
--
--      문자 클래스마다 A-F 를 넣는 방법도 있지만 클래스가 다섯이라 하나만 빠뜨려도
--      같은 구멍이 남는다. ~* 는 한 글자로 "대소문자를 보지 않는다" 는 의도를 드러내고
--      빠뜨릴 자리가 없다. ::uuid 캐스팅이 대문자를 소문자로 정규화하므로 저장되는
--      값은 어느 쪽이든 같다. (외부 리뷰 CodeRabbit)
UPDATE upload_audit
   SET file_id = CASE
